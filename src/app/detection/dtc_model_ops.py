...
class DtcModelOperations:
    ...
    def train_model(self):
        print("Detection model training is starting.")
        df = pd.read_csv(self.dataset_path, encoding='utf-8')

        # --- Proto ENCODING ---
        df['Proto'] = self.proto_encoder.fit_transform(df['Proto'])
        ...
        print("Detection model training is done.")
        return model
    ...
    def save_results(self, model, m, cm_df, class_report, mcm_str):
        ...
        for path in [self.proto_encoder_path]:
            joblib.dump(self.proto_encoder, path)
        for path in [self.label_encoder_path]:
            joblib.dump(self.label_encoder, path)
        with open(self.encoders_map_path, 'w', encoding='utf-8') as f:
                f.write(f"Proto Encoding Map: {proto_map}\n"
            f"Detection Model Label Encoding Map: {label_map}\n")
    ...
    def load_model(self):
        if os.path.exists(self.model_file):
            print(f"Detection model loaded: {self.model_file}")
